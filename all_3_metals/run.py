...
Ternary batch pipeline (two metals + one ligand):
- All unordered pairs of d-block transition metals (M1-M2)
- Ligands: O and S
- For each unique pair M1-M2 and ligand L, query Materials Project for chemsys "M1-M2-L"
- Parallel: metal-pair×ligand tasks in parallel; strains per POSCAR in parallel
- Checkpointing: skip chemsys if aggregated CSV already exists

Notes:
- By default this uses unordered distinct-metal pairs (M1 != M2 and M1-M2 == M2-M1).
  If you want to allow identical-metal pairs (M1==M2), set ALLOW_IDENTICAL_METAL_PAIRS = True

This is adapted from the original parallel_hardcoded.py to support ternary searches of the
form "M1-M2-O" or "M1-M2-S".
"""
...
# ---- Materials Project API key sources ----
...
ALLOW_IDENTICAL_METAL_PAIRS = False
...
# ---- DFTD3 ----
S_DFTD3_CMD = "/home/cyt101_09/miniforge3/envs/mpwork/bin/s-dftd3"
TIMEOUT_RUN = 900

# ---- Strain grid ----
STRAIN_INDICES = list(range(-5, 30))  # -5, -4, ..., 29

# ---- MP API throttle ----
SLEEP_BETWEEN_API = 0.25
